...
const Compare: React.FC = () => {
  const { left } = useParams<{ left: string, right: string }>()
  const { loading, error, data } = useQuery(GET_DATA, { variables: { id: left } })
    ...
    <Container sx={{ mt: 4 }} maxWidth='xl'>
      <Grid container spacing={4} sx={{ flexDirection: { md: 'row-reverse' } }}>
        ...
        <Grid item xs={12} md={4} sx={{ maxWidth: { md: '300px' }, position: { md: 'fixed' } }}>
          <ItemCard title={title} description={description} image={others.images[0]} id={left!} />
        </Grid>
